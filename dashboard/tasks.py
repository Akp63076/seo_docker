...
def update_keyword_table(path, conn):
    ...
    try:
        ...
        if df.empty != True:
            ...
            sql = """
                UPDATE dashboard_keyword_table AS f
            ...
            sleep(1)
            unique_upload(df1, 'dashboard_keyword_table',
...
def update_keyword_frequency_table(path,conn,frequency):
    ...
    data2=dict(conn.execute("select keyword, id from dashboard_keyword_table ").fetchall())
    ...
    unique_upload(df,'dashboard_keyword_frequency_table',conn)
...
def update_tag_table(path,conn):
    ...
    try:
        ...
        data1=dict(conn.execute("select tag, id from dashboard_tag_table ").fetchall())
        data2=dict(conn.execute("select keyword, id from dashboard_keyword_table ").fetchall())
        ...
        sql=""" delete from dashboard_keyword_tag_table where
         tag_id in (select id from dashboard_tag_table where tag='Undefined')
          and keyword_id in (select keyword_id from temp_table)"""
        conn.execute(sql)
        conn.execute("DROP Table temp_table")
        sleep(1)
        unique_upload(df,'dashboard_keyword_tag_table',conn)
    except Exception as e :
            logger.error(e)
def update_rel_search_table(path,conn):
    try:
        ...
        if data1.empty != True:
            ...
            df = data1[['rel_search']]
            unique_upload(df, 'dashboard_rel_search_table',
                          conn, data_type=data_type)

            #"""key_rel_search_table udation"""#
            data2 = dict(conn.execute(
                "select rel_search, id from dashboard_rel_search_table ").fetchall())
            data3 = dict(conn.execute(
                "select keyword, id from dashboard_keyword_table ").fetchall())
            ...
            unique_upload(df, 'dashboard_keyword_rel_search_table',
                          conn, data_type=data_type)
    except Exception as e:
        logger.error(e)
...
def update_domain_table(path, conn):
    try:
        ...
        if df.empty != True:
            ...
            unique_upload(df, 'dashboard_domain_table',
                          conn, data_type=data_type)
    except Exception as e:
        logger.error(e)
    ...
    try:
        ...
        if data1.empty != True:
            ...
            domain_id = dict(conn.execute(
                "select domain, id from dashboard_domain_table ").fetchall())
            ...
            unique_upload(df, 'dashboard_rel_question_table',
                          conn, data_type=data_type)

            #"""update of key_rel_question_table updation"""#
            df = data1[['keyword', 'question']]
            question_id = dict(conn.execute(
                "select rel_question, id from dashboard_rel_question_table ").fetchall())
            ...
            unique_upload(df, 'dashboard_keyword_rel_question_table',
                          conn, data_type=data_type)
    except Exception as e:
        logger.error(e)
...
def update_description_table(path, conn,frequency):
    try:
        ...
        if data1.empty != True:
            ...
            query = """select DISTINCT(desc_id), keyword_id,pos, url,  title, url_shown,pos_overall, domain_id,description, date from public."myTempTable"  WHERE desc_id NOT IN (select distinct(desc_id) from dashboard_description_table)
            Except
            select desc_id,keyword_id ,pos, url,  title, url_shown,pos_overall, domain_id,description, date from dashboard_description_table"""
            new_data = pd.read_sql(query, con=conn)
            ##print(len(df.index)- len(new_data.index) ,"  data are skipped due to duplicate data")
            new_data.to_sql('dashboard_description_table', con=conn,
                            index=False, if_exists='append', dtype=data_type)
            print("dashboard_description_table update successfully \n\n")
...
def update_question_table(path, conn):
    try:
        ...
        if data1.empty != True:
            ...
            query = """select question ,pos, desc_id  from public."myTempTable" where desc_id in (select desc_id from dashboard_description_table)
            Except 
            select  question ,pos,desc_id   from dashboard_question_table"""
            data_type = {'question': sqlalchemy.String,
                         'pos': sqlalchemy.Integer, 'desc_id': sqlalchemy.String}
            unique_upload(df, 'dashboard_question_table',
...
def update_sitelink_table(path, conn):
    try:
        ...
        if data1.empty != True:
            ...
            query = """select url_type,url,title,desc_id from public."myTempTable"  where desc_id in (select desc_id from dashboard_description_table)
            Except 
            select url_type,url,title,desc_id from dashboard_sitelink_table"""
            data_type = {'url_type': sqlalchemy.String, 'url': sqlalchemy.String,
                         'title': sqlalchemy.String, 'desc_id': sqlalchemy.String}

            unique_upload(df, 'dashboard_sitelink_table',
                          conn, query, data_type=data_type)
    except Exception as e:
        logger.error(e)
